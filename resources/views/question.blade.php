            ...
            <div class="col-md-9 col-md-push-3">
                <div class="panel panel-default">
                        ...
                        @if ((isset(Auth::user()->id) && Auth::user()->id != $question->user_id) && in_array(Auth::user()->id,$answer_ids) == FALSE)
                            <id id="post-answer">
                                @if( isset(Auth::user()->id) && !Auth::check())
                                    <p>Пожалуйста <a href="/login">авторизируйтесь</a> для отправки данного поста!</p>
                                @else
                    ...
                    <!-- END Show the answer form -->
                        @if ( !$answers->isEmpty() )
                            <div id="answers">
                                <legend class="text-left">Ответы</legend>
                            </div>
                        ...
                        @if ( $recent_questions->isEmpty() )
                            <p>Вопросы не соответствующие правилам сайта</p>
                        @else
                            <div id="questions">
                                <legend class="text-left" style="font-size: 24px;">Релевантные вопросы</legend>
                            </div>
